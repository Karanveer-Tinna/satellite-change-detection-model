...
def get_model(model_name, device, encoder_name='resnet101', encoder_weights='imagenet', classes=7):
    ...
    Args:
        model_name (str): The name of the model ('unetplusplus', 'deeplabv3plus', 'pspnet').
        device (torch.device): The device to move the model to.
        encoder_name (str): Name of the encoder architecture (e.g., 'resnet101').
    ...
    if model_name.lower() == 'unetplusplus':
        ...
        print(f"Initializing PSPNet model with encoder {encoder_name}.")
        model = smp.PSPNet(
            encoder_name=encoder_name, encoder_weights=encoder_weights, in_channels=3, classes=classes
        ).to(device)
    else:
        raise ValueError(f"Model '{model_name}' is not supported. Choose from 'unetplusplus', 'deeplabv3plus', or 'pspnet'.")
    return model
