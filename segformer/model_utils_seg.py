...
def get_model(model_name, device, encoder_name, encoder_weights='imagenet', classes=7):
    """
    Initializes a segmentation model based on the provided model and encoder names.
    
    Args:
        model_name (str): The name of the model architecture (e.g., 'Segformer').
        device (torch.device): The device to move the model to.
        encoder_name (str): Name of the encoder backbone (e.g., 'mit_b5').
    ...
    print(f"Initializing {model_name.upper()} model with {encoder_name} encoder.")
    ...
    if model_name == 'segformer':
        ...
        raise ValueError(f"Model '{model_name}' is not supported. Choose from 'Segformer', 'UnetPlusPlus', etc.")
